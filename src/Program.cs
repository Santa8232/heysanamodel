...
class Program
{
    static void Main(string[] args)
    {
        ...
        // Check if user requested ONNX inspection or test
        if (args.Length > 0 && (args[0] == "--onnx" || args[0] == "-o" || args[0] == "--inspect-onnx"))
        {
            RunOnnxInspection(args, baseDir);
            return;
        }
    ...
    static void RunOnnxInspection(string[] args, string baseDir)
    {
        string onnxPath = Path.Combine(baseDir, "..", "..", "..", "..", "artifacts", "model.onnx");
        if (!File.Exists(onnxPath))
        {
            onnxPath = Path.Combine(Directory.GetCurrentDirectory(), "artifacts", "model.onnx");
        }

        if (!File.Exists(onnxPath))
        {
            Console.WriteLine($"Error: ONNX model not found at {onnxPath}");
            return;
        }

        var fileInfo = new FileInfo(onnxPath);
        Console.WriteLine("==================================================");
        Console.WriteLine("       HeySanaModel - ONNX Model Inspector        ");
        Console.WriteLine("==================================================");
        ...
        Console.WriteLine("\n📥 Input Nodes (Features required by ONNX model):");
        ...
        Console.WriteLine("\n📤 Output Nodes (Predictions generated by ONNX model):");
        ...
        // Scenario A: Custom arguments passed to ONNX inference
        // Format: dotnet run -- --onnx <Age> <Days> <Type> <Activity> <Budget> [Season] [Fitness] [Stay]
        if (args.Length >= 6)
        {
            float age = float.Parse(args[1]);
            float days = float.Parse(args[2]);
            ...
            var pred = RunOnnxPredict(session, input);
            DisplayRichRecommendation(input, pred);
            return;
        }

        // Scenario B: Default Inspector & Live Sample Verification
        Console.WriteLine("\n🧪 Running Live ONNX Inference Verification on Sample Profile:");
        Console.WriteLine("   Inputs: Age=24, DurationDays=4, Type=Solo, Activity=Trekking, Budget=160, Season=Summer, Fitness=Active, Stay=Camping");

        var sampleInput = new TravellerData
        {
            Age = 24,
            DurationDays = 4,
            TravelerType = "Solo",
        ...
        var samplePred = RunOnnxPredict(session, sampleInput);
        DisplayRichRecommendation(sampleInput, samplePred);

        Console.WriteLine("✅ ONNX Model Runtime Verification Completed Successfully!");
        Console.WriteLine("==================================================\n");
    }

    static TravellerPrediction RunOnnxPredict(InferenceSession session, TravellerData input)
    {
        var ageTensor = new DenseTensor<float>(new[] { input.Age }, new[] { 1, 1 });
        var durationTensor = new DenseTensor<float>(new[] { input.DurationDays }, new[] { 1, 1 });
        var typeTensor = new DenseTensor<string>(new[] { input.TravelerType }, new[] { 1, 1 });
        var activityTensor = new DenseTensor<string>(new[] { input.PreferredActivity }, new[] { 1, 1 });
        var budgetTensor = new DenseTensor<float>(new[] { input.BudgetUSD }, new[] { 1, 1 });
        var seasonTensor = new DenseTensor<string>(new[] { input.Season }, new[] { 1, 1 });
        var fitnessTensor = new DenseTensor<string>(new[] { input.FitnessLevel }, new[] { 1, 1 });
        var stayTensor = new DenseTensor<string>(new[] { input.StayPreference }, new[] { 1, 1 });
        var visitedPlaceTensor = new DenseTensor<string>(new[] { "" }, new[] { 1, 1 });

        var inputs = new List<NamedOnnxValue>
        {
            NamedOnnxValue.CreateFromTensor("Age", ageTensor),
            NamedOnnxValue.CreateFromTensor("DurationDays", durationTensor),
            NamedOnnxValue.CreateFromTensor("TravelerType", typeTensor),
            NamedOnnxValue.CreateFromTensor("PreferredActivity", activityTensor),
            NamedOnnxValue.CreateFromTensor("BudgetUSD", budgetTensor),
            NamedOnnxValue.CreateFromTensor("Season", seasonTensor),
            NamedOnnxValue.CreateFromTensor("FitnessLevel", fitnessTensor),
            NamedOnnxValue.CreateFromTensor("StayPreference", stayTensor),
            NamedOnnxValue.CreateFromTensor("VisitedPlace", visitedPlaceTensor)
        };

        using var results = session.Run(inputs);

        string predictedPlace = "Manipur";
        float[] scores = Array.Empty<float>();

        foreach (var r in results)
        {
            if (r.Name.Contains("PredictedLabel"))
            {
                predictedPlace = r.AsTensor<string>().GetValue(0);
            }
            if (r.Name == "Score.output")
            {
                scores = r.AsTensor<float>().ToArray();
            }
        }

        return new TravellerPrediction
        {
